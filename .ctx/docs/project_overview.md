...
## Building & packaging
- **Windows:** GitHub Actions (`.github/workflows/build.yml`) builds with PyInstaller and wraps it in an Inno Setup installer (`installer.iss`, `dicto.spec`). `make exe` / `make installer` use the same spec, and the installer version is synced from `pyproject.toml`.
...
- **Self-update:** a silent check runs a few seconds after startup; if a newer release exists the app flags it with a dot on the settings gear, a tray menu entry and a desktop notification (it never interrupts on failure, so an offline start is quiet). From Settings → Updates the user can also check on demand, and install in place on frozen builds. On the Linux `.deb` it downloads the new `.deb` and installs it through `pkexec` (PolicyKit auth prompt); on Windows it downloads the Inno Setup `Dicto-<ver>-setup.exe`, runs it silently and exits so the installer can replace the locked files and relaunch the app. When in-place install isn't possible (e.g. the portable `.tar.gz`) it opens the release download page instead. PyInstaller builds pass `--copy-metadata dicto` so the bundled app knows its own version (`src/version.py`).
