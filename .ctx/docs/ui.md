...
- `src/dicto/ui/theme/manager.py` - `ThemeManager`: reads OS preference via Windows registry, builds the full design-system stylesheet (buttons incl. `ghost`/`iconBtn`/`chip`/`rail` variants, inputs, list cards, tabs, menus, scrollbars), emits `themeChanged`, supports live switching. On init it registers the bundled UI font and leads the QSS `font-family` stack with it
- `src/dicto/ui/theme/fonts.py` - `load_bundled_fonts()`: registers the bundled Hanken Grotesk variable font (`theme/fonts/HankenGrotesk-Variable.ttf`, OFL) with Qt via `QFontDatabase` and returns its family name; degrades to the system sans (`None`) if the file is missing or Qt rejects it. Ships inside the package dir so the wheel/PyInstaller build carries it
...
- `src/dicto/ui/main/window.py` - `MainWindow`: 58px icon rail (round red record button, library, dictionary, settings, avatar — emits `recordRequested`/`dictionaryRequested`/`settingsRequested`) + fixed 344px library column + a right pane that stacks the `DetailView` and `ChatView` (the detail's Ask tab flips to chat; a new selection snaps back to detail); `refresh_library()` reloads the list after an auto-save; closing hides to tray
...
- `src/dicto/ui/main/detail_view.py` - `DetailView`: large title + actions (Edit toggle reveals Save + tags row; Copy / Export icon buttons), meta row, tab bar (Transcript + transform tabs: summary/key-points/flashcards/rewrite each generated on demand via `TransformService` and cached; the Ask tab emits `askRequested` to open the chat). The transform page has a result header (✦ preset name · cached chip · Generate/Regenerate) over a scroll of structured content (`transform_render`); footer (insert-at-cursor · cleanup-on · word count · custom-prompt); emits `saved` / `statusMessage` / `askRequested`
- `src/dicto/ui/main/chat_view.py` - `ChatView`: "ask your notes" — heading, a scroll of chat bubbles (user right/accent, AI left/elevated, per the design `.dx-bubble`), question input + Ask button. Runs the `ask` preset grounded in one transcript (answers not cached); shares the right pane with `DetailView` via a `QStackedWidget` in the window
- `src/dicto/ui/main/transform_render.py` - `render_result(preset_id, text)`: turns a transform's plain text into the design's structured widget — flashcards as a 2-col card grid, key points as a numbered list, summary/rewrite as prose; parses leniently and falls back to prose
- `src/dicto/ui/main/transform_worker.py` - `run_transform`: runs a transform call on the global `QThreadPool` and delivers the result/error back on the GUI thread (keeps the network off the Qt thread)
