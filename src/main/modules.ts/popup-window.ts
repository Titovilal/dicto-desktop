...
export function showRecordingPopup(options: PopupState | PopupOptions): void {
  if (!popupWindow) {
...
export function hideRecordingPopup(): void {
  if (popupWindow && popupWindow.isVisible()) {
    popupWindow.hide()
  }
}

export function destroyRecordingPopup(): void {
  if (popupWindow) {
    try {
      popupWindow.destroy()
      popupWindow = null
...
export function updatePopupMessage(message: string): void {
  if (popupWindow) {
    try {
...
type PopupWindowReturn = {
  showRecordingPopup: (options: PopupState | PopupOptions) => void
  updatePopupState: (state: PopupState) => void
  hideRecordingPopup: () => void
  destroyRecordingPopup: () => void
  updatePopupMessage: (message: string) => void
}

// Actualizar el registro del manejador IPC
export function initPopupWindow(): PopupWindowReturn {
